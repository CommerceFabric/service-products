...
This document describes how the `CommerceFabric` microservice repositories automatically build and publish Docker images to:
...
For example, `service-products` contains:

```text
service-products/
├── CommerceFabric.ProductService/
│   ├── Dockerfile
│   ├── CommerceFabric.ProductService.API.csproj
│   └── ...
└── .github/
    └── workflows/
        └── docker.yml
...
```yaml
docker build \
  --file CommerceFabric.ProductService/Dockerfile \
  ...
  .
```
...
For `CommerceFabric/service-products`:
...
$parameters = @'
{
  "name": "github-service-products-main",
  "issuer": "https://token.actions.githubusercontent.com",
  "subject": "repo:CommerceFabric/service-products:ref:refs/heads/main",
  "description": "GitHub Actions for CommerceFabric/service-products main branch",
  "audiences": [
...
repo:CommerceFabric/service-users:ref:refs/heads/main
```
...
```text
CommerceFabric.ProductService/Dockerfile
```
...
docker build \
  --file CommerceFabric.ProductService/Dockerfile \
  --tag commercefabricregistry.azurecr.io/products-microservice:$PRODUCT_VERSION \
  --tag danielmusselwhite/commercefabric_product_microservice:$PRODUCT_VERSION \
  .
```
...
commercefabricregistry.azurecr.io/products-microservice:<version>
```

### Docker Hub

```text
danielmusselwhite/commercefabric_product_microservice:latest
danielmusselwhite/commercefabric_product_microservice:<version>
```
...
```text
GitHub
CommerceFabric/service-products
          │
          │ Push to main
          ▼
    GitHub Actions
          ...
          └──────── PAT ─────────► Docker Hub
                                      │
                                      ▼
                    danielmusselwhite/commercefabric_product_microservice
```
