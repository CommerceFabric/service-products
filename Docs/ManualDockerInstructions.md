...
```bash
docker run -d --name mysql-productservice --network productsmicroservice-network -e MYSQL_ROOT_PASSWORD=admin -e MYSQL_DATABASE=productService -v ./Resources/docker/mysql-init:/docker-entrypoint-initdb.d -p 3306:3306 mysql:9.7.1
```
